...
encode_phosphoprimer_location <- function(sequence) {
    ...
    str_split(cleaned, "") |> # nolint: fixed_regex_linter.
        map_chr(
            \(split_string) { # nolint: unnecessary_nesting_linter.
...
encode_phosphosite_location <- function(sequence) {
    index <- str_locate(sequence, "[STY]\\*")[, 1L]
...
encode_aa_disambiguation <- function(sequence) {
    cleaned <- str_remove_all(sequence, "\\W")
    case_when(
        str_detect(cleaned, fixed("SENT")) ~ "B3xN",
        str_detect(cleaned, fixed("SEDT")) ~ "B3xD",
        .default = "O0xO"
    )
}
...
handle_ambiguous_aa <- function(seq) {
    ...
    if (str_detect(seq, fixed("B"))) {
        seq_n <- str_replace(seq, fixed("B"), "N")
        seq_d <- str_replace(seq, fixed("B"), "D")
        return(list(seq_n, seq_d))
    }

    # Check for Z (glutamine or glutamic acid)
    if (str_detect(seq, fixed("Z"))) {
        seq_e <- str_replace(seq, fixed("Z"), "E")
        seq_q <- str_replace(seq, fixed("Z"), "Q")
...
mark_phosphosites <- function(seq, chip) {
    if (chip == "STK") {
        s_positions <- str_locate_all(seq, "[ST]")[[1L]][, 1L]
    } else if (chip == "PTK") {
        s_positions <- str_locate_all(seq, "[Y]")[[1L]][, 1L] # nolint: fixed_regex_linter.
    }

    sequences <- rep(NA, length(s_positions))

    for (idx in seq_along(sequences)) {
